...
def auto_classify_error(
    ...
    """Automatically classify an error after analyzing it. Call this EVERY TIME
    you finish analyzing errors to build the AI learning knowledge base.

    The classification persists across conversations. Over time this builds a
    comprehensive map of which errors are real problems vs noise, enabling
    instant triage for recurring errors.

    Args:
        error_pattern: The error message or pattern (be specific, include key identifiers).
        classification: One of:
            - 'actionable' — Real issue that needs human attention
            - 'noise' — Log noise, benign, not a real problem
            - 'transient' — Temporary issue that self-resolves (retries, brief network blips)
            - 'known_issue' — Known bug or limitation being tracked
            - 'configuration' — Misconfiguration that can be fixed via config change
            - 'dependency' — Caused by external dependency failure
        severity: One of: 'critical', 'high', 'medium', 'low', 'noise'
        confidence: 0.0 to 1.0 — how confident you are in this classification.
        application: Which application (e.g. "myaccount", "sdp").
        environment: Which environment (e.g. "dev", "qa", "prod").
        root_cause: Brief root cause hypothesis.
        suggested_action: What should be done about this error.
        occurrence_count: How many times this error was seen in the current analysis.
    """
    valid_classifications = {"actionable", "noise", "transient", "known_issue", "configuration", "dependency"}
    ...
    # Check if we've already classified something very similar
    existing = _get_kb().search_classifications(error_pattern, top_k=3)
    for ex in existing:
        if ex.get("distance", 99) < 0.3:  # Very close match
            # Update occurrence count instead of creating duplicate
            old_count = int(ex["metadata"].get("occurrence_count", "1"))
            _get_kb().update_classification_count(ex["id"], old_count + occurrence_count)
            old_class = ex["metadata"].get("classification", "unknown")
            old_sev = ex["metadata"].get("severity", "unknown")
            return (
                f"Updated existing classification (id={ex['id']}).\n"
                f"Pattern: {error_pattern[:120]}\n"
                f"Classification: {old_class} | Severity: {old_sev}\n"
                f"Total occurrences: {old_count + occurrence_count} | Times classified: +1\n"
                f"This error has been seen before — the AI is reinforcing its understanding."
            )

    # Store new classification
    doc_id = _get_kb().store_classification(
        error_pattern=error_pattern,
    ...
    class_emoji = {
        "actionable": "⚡", "noise": "💤", "transient": "🔄",
        "known_issue": "📌", "configuration": "⚙️", "dependency": "🔗"
    }

    return (
        f"{severity_emoji.get(severity, '❓')} {class_emoji.get(classification, '❓')} "
        f"Classified error (id={doc_id}):\n"
        f"Pattern: {error_pattern[:150]}\n"
        f"Classification: **{classification}** | Severity: **{severity}** | "
        f"Confidence: {confidence:.0%}\n"
        f"{'Root cause: ' + root_cause[:150] if root_cause else ''}\n"
        f"{'Action: ' + suggested_action[:150] if suggested_action else ''}\n"
        f"This classification is now stored — I'll recognize this error pattern in future analyses."
    )


@tool
def get_error_intelligence(error_text: str) -> str:
    """Check what the AI has previously learned about an error pattern.

    This combines both AI auto-classifications AND user feedback to provide
    complete intelligence about an error. Call this before analyzing errors
    to leverage accumulated learning.

    Args:
        error_text: The error message or pattern to look up.
    """
    # Check AI classifications
    classifications = _get_kb().search_classifications(error_text, top_k=5)
    close_classifications = [c for c in classifications if c.get("distance", 99) < 1.5]

    # Also check user feedback
    feedback = _get_kb().search_feedback(error_text, top_k=5)
    close_feedback = [f for f in feedback if f.get("distance", 99) < 1.5]

    if not close_classifications and not close_feedback:
        return (
            "No prior intelligence found for this error pattern. "
            "This appears to be a NEW error the AI hasn't seen before. "
            "After analyzing it, use `auto_classify_error` to store what you learn."
        )

    lines = ["**Error Intelligence Report:**\n"]

    if close_classifications:
        lines.append("### AI Classifications:")
        for c in close_classifications:
            meta = c["metadata"]
            ...
            times = meta.get("times_classified", "1")
            first_seen = meta.get("first_seen", "")[:10]
            last_seen = meta.get("last_seen", "")[:10]

            severity_emoji = {"critical": "🔴", "high": "🟠", "medium": "🟡", "low": "🔵", "noise": "⚪"}.get(sev, "❓")
            class_emoji = {"actionable": "⚡", "noise": "💤", "transient": "🔄", "known_issue": "📌", "configuration": "⚙️", "dependency": "🔗"}.get(cls, "❓")

            line = f"{severity_emoji} {class_emoji} **{cls}** (severity: {sev}, confidence: {conf:.0%})"
            if app:
                line += f" | App: {app}"
            if env:
                line += f" | Env: {env}"
            line += f"\n  Pattern: {meta.get('error_pattern', '')[:120]}"
            line += f"\n  Seen {count}× | Classified {times}× | First: {first_seen} | Last: {last_seen}"
            if rc:
                line += f"\n  Root cause: {rc[:150]}"
            if action:
                line += f"\n  Suggested action: {action[:150]}"
            lines.append(line)

    if close_feedback:
        lines.append("\n### User Feedback:")
        for f in close_feedback:
            meta = f["metadata"]
            fb_type = meta.get("feedback_type", "unknown")
            app = meta.get("application", "")
            note = meta.get("user_note", "")
            resolution = meta.get("resolution", "")
            emoji = {"known_issue": "🔵", "expected": "✅", "noise": "⚪", "resolved": "🟢", "critical": "🔴"}.get(fb_type, "❓")
            line = f"{emoji} User marked as **{fb_type.replace('_', ' ')}**"
            if app:
                line += f" ({app})"
            if note:
                line += f" — {note}"
...
@tool
def get_classification_summary(application: str = "") -> str:
    """Get a summary of all AI error classifications, showing what the AI has
    learned over time. This is the AI's accumulated understanding of the error
    landscape.

    Args:
        application: Optional — filter by application name. Leave empty for all.
    """
    items = _get_kb().list_all_classifications(application=application)
    if not items:
        return (
            "No AI error classifications stored yet. The AI hasn't analyzed any errors yet.\n"
            "As errors are analyzed, the AI will automatically classify them and build "
            "up a knowledge base of which errors are real issues vs noise."
        )

    # Group by classification type
    groups: dict[str, list] = {}
    for it in items:
        cls = it["metadata"].get("classification", "unknown")
        groups.setdefault(cls, []).append(it)

    # Stats
    total = len(items)
    severity_counts: dict[str, int] = {}
    for it in items:
        sev = it["metadata"].get("severity", "unknown")
        severity_counts[sev] = severity_counts.get(sev, 0) + 1

    lines = [f"**AI Error Intelligence Summary** ({total} classified patterns)\n"]

    # Severity breakdown
    sev_order = ["critical", "high", "medium", "low", "noise"]
    sev_emoji = {"critical": "🔴", "high": "🟠", "medium": "🟡", "low": "🔵", "noise": "⚪"}
    sev_parts = []
    for s in sev_order:
        if s in severity_counts:
            sev_parts.append(f"{sev_emoji.get(s, '')} {s}: {severity_counts[s]}")
    lines.append("Severity: " + " | ".join(sev_parts))

    # Classification breakdown
    class_emoji = {"actionable": "⚡", "noise": "💤", "transient": "🔄", "known_issue": "📌", "configuration": "⚙️", "dependency": "🔗"}
    for cls_type in ["actionable", "critical", "noise", "transient", "known_issue", "configuration", "dependency"]:
        items_in_group = groups.get(cls_type, [])
        if not items_in_group:
            continue
        emoji = class_emoji.get(cls_type, "❓")
        lines.append(f"\n### {emoji} {cls_type.replace('_', ' ').title()} ({len(items_in_group)})")
        for it in items_in_group[:5]:  # Top 5 per group
            meta = it["metadata"]
            pattern = meta.get("error_pattern", "")[:100]
            sev = meta.get("severity", "")
            count = meta.get("occurrence_count", "1")
            app = meta.get("application", "")
            lines.append(f"  - [{sev}] {pattern} (seen {count}×){' — ' + app if app else ''}")
        if len(items_in_group) > 5:
            lines.append(f"  ... and {len(items_in_group) - 5} more")
...
def lookup_application(app_name: str) -> str:
    ...
    if entry:
        queries = entry.get("splunk_queries", [{"index": entry["index"], "sourcetype": entry["sourcetype"], "label": "default"}])
        return json.dumps({
            "found": True,
            "app_name": app_name,
            "index": entry["index"],
...
def query_app_errors(
    ...
    # Query all Splunk index/sourcetype pairs for this app and merge
    broad_search = "error"
    ...
    for q in queries:
        try:
            app_source = SplunkSource(index=q["index"], sourcetype=q["sourcetype"])
            ...
            queried_labels.append(f"{q.get('label', q['index'])} ({len(broad_entries)+len(focused_entries)} raw)")
        except Exception as exc:
            queried_labels.append(f"{q.get('label', q['index'])} (error: {exc})")
...
def query_app_logs(
    ...
    for q in queries:
        try:
            app_source = SplunkSource(index=q["index"], sourcetype=q["sourcetype"])
            q_entries = app_source.query(st, et, search_query or None, max_results)
            for e in q_entries:
                key = f"{e.timestamp.isoformat()}|{e.message[:100]}"
                if key not in seen:
                    seen.add(key)
                    all_log_entries.append(e)
            queried_labels.append(f"{q.get('label', q['index'])} ({len(q_entries)})")
        except Exception as exc:
            queried_labels.append(f"{q.get('label', q['index'])} (error: {exc})")
    ...
    return header + _entries_to_text(all_log_entries)
